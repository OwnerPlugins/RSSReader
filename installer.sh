...
echo "Starting RSSReader installation..."

if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/RSSReader
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/RSSReader
fi
...
cleanup() {
    ...
    [ -d "/tmp/RSSReader-main" ] && rm -rf "/tmp/RSSReader-main"
}
...
if [ -d "$TMPPATH/RSSReader-main/usr/lib/enigma2/python/Plugins/Extensions/RSSReader" ]; then
    cp -r "$TMPPATH/RSSReader-main/usr/lib/enigma2/python/Plugins/Extensions/RSSReader"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/RSSReader-main/usr/lib64/enigma2/python/Plugins/Extensions/RSSReader" ]; then
    cp -r "$TMPPATH/RSSReader-main/usr/lib64/enigma2/python/Plugins/Extensions/RSSReader"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/RSSReader-main/usr" ]; then
    cp -r "$TMPPATH/RSSReader-main/usr"/* /usr/ 2>/dev/null
